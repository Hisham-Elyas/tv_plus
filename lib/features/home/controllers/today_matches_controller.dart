...
class TodayMatchesController extends GetxController {
  ...
  void onInit() async {
    ...
    await loopAndFindChannels(matches);
  }

  Future<Channel> _findChannelByName2(String channelName) async {
  ...
  Future<void> loopAndFindChannels(List<MatchModel> matches) async {
    for (var match in matches) {
      if (match.channelsAndCommentators.isNotEmpty) {
        final channelName = match.channelsAndCommentators.first.channel;

        try {
          print("==================================================");
          final foundChannel = await _findChannelByName2(channelName);

          print("🎯 Original Channel: $channelName");

          print("✅ Found Channel: ${foundChannel.name}");
          print("🏆 League: ${match.league}");
          print("⚽ Match: ${match.homeTeam} vs ${match.awayTeam}\n");
        } catch (e) {
          print("❌ Channel not found for: $channelName");
          print("🏆 League: ${match.league}");
          print("⚽ Match: ${match.homeTeam} vs ${match.awayTeam}\n");
        }
      } else {
        print("⚠️ No channel info for match:");
        print("🏆 League: ${match.league}");
        print("⚽ Match: ${match.homeTeam} vs ${match.awayTeam}\n");
      }
    }
...
// timer countdown  countdown match
  ...
  String normalizeChannelName(String name) {
    return name
        ...
        .replaceAll(RegExp(r'ᴴᴰ'), '') // Remove "HD"
